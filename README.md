<div align="center">
  <h1>🧮 Awesome Calculators</h1>
  <p>A curated list of awesome online calculators, tools, and resources.</p>
  <p><em>Find the right calculator — fast, free, no sign-up required.</em></p>
  <p>
    <a href="README.md"><img src="https://img.shields.io/badge/EN-English-blue?style=flat-square" alt="English"></a>
    <a href="README.zh-CN.md"><img src="https://img.shields.io/badge/ZH-简体中文-red?style=flat-square" alt="简体中文"></a>
    <a href="README.ja.md"><img src="https://img.shields.io/badge/JA-日本語-white?style=flat-square" alt="日本語"></a>
    <a href="README.ko.md"><img src="https://img.shields.io/badge/KO-한국어-purple?style=flat-square" alt="한국어"></a>
  </p>
  <p>
    <a href="#contributing"><img src="https://img.shields.io/badge/PRs-welcome-brightgreen.svg" alt="PRs Welcome"></a>
    <a href="LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue.svg" alt="MIT License"></a>
  </p>
</div>

<br>

## 📋 Contents

- [Multi-Category Platforms](#multi-category-platforms)
- [Finance & Investment](#finance--investment)
- [Health & Fitness](#health--fitness)
- [Math & Statistics](#math--statistics)
- [Science & Engineering](#science--engineering)
- [Unit Conversion](#unit-conversion)
- [Everyday Life](#everyday-life)
- [Privacy-First Tools](#privacy-first-tools)
- [Contributing](#contributing)

---

## Multi-Category Platforms

_All-in-one calculator platforms covering multiple domains._

- [**Calnex**](https://calnex.pro) — 147+ free calculators covering finance, health, math, physics, conversions, and daily life. All calculations run **locally in your browser** — zero data upload, no registration needed. 12 language locales. ⭐

## Finance & Investment

- [Mortgage Calculator](https://www.calculator.net/mortgage-calculator.html) — Classic mortgage payment estimator.
- [Compound Interest Calculator](https://www.investor.gov/financial-tools-calculators/calculators/compound-interest-calculator) — Official SEC compound interest tool.
- [Compound Interest Calculator (Interactive)](https://calnex.pro/en_US/calculators/finance/compound_interest_calculator) — Visual compound interest with charts, daily/monthly/yearly contributions.
- [Loan Calculator](https://www.bankrate.com/loans/loan-calculator/) — Bankrate loan amortization.
- [WACC Calculator](https://calnex.pro/en_US/calculators/finance/wacc_calculator) — Weighted average cost of capital for business.
- [ROI Calculator](https://www.omnicalculator.com/finance/roi) — Return on investment estimator.
- [Coffee Compound Shocker](https://calnex.pro/en_US/calculators/finance/coffee_compound) — See the shocking opportunity cost of daily coffee spending.
- [Discount & Markup Calculator](https://www.omnicalculator.com/finance/markup) — Calculate discounts and markups.

## Health & Fitness

- [BMI Calculator](https://www.nhlbi.nih.gov/health/educational/lose_wt/BMI/bmicalc.htm) — Official NIH BMI calculator.
- [BMI Calculator](https://calnex.pro/en_US/calculators/health/bmi_calculator) — BMI with visual categorization, metric & imperial.
- [BMR Calculator](https://calnex.pro/en_US/calculators/health/bmr_calculator) — Basal metabolic rate with Mifflin-St Jeor & Harris-Benedict equations.
- [Body Fat Calculator](https://www.omnicalculator.com/health/body-fat) — Body fat percentage estimation.
- [Calorie Calculator](https://calnex.pro/en_US/calculators/health/calorie_calculator) — Daily calorie needs based on activity level and goals.
- [Pregnancy Due Date Calculator](https://www.whattoexpect.com/due-date-calculator/) — Pregnancy due date estimator.
- [Macro Calculator](https://calnex.pro/en_US/calculators/health/macro_calculator) — Protein, carbs, and fat targets by fitness goal.
- [Heart Rate Zone Calculator](https://calnex.pro/en_US/calculators/health/heart_rate_zone_calculator) — Training zones based on max heart rate.

## Math & Statistics

- [Desmos Graphing Calculator](https://www.desmos.com/calculator) — Advanced interactive graphing.
- [Wolfram Alpha](https://www.wolframalpha.com/) — Computational knowledge engine.
- [Normal Distribution Calculator](https://calnex.pro/en_US/calculators/math/normal_distribution_calculator) — Gaussian distribution with interactive charts.
- [Z-Score Calculator](https://calnex.pro/en_US/calculators/math/z_score_calculator) — Standard score & percentile computation.
- [Sample Size Calculator](https://calnex.pro/en_US/calculators/math/sample_size_calculator) — Statistical sample size for surveys & research.
- [Matrix Calculator](https://matrix.reshish.com/) — Matrix operations & determinant calculator.
- [Symbolab](https://www.symbolab.com/) — Step-by-step math equation solver.

## Science & Engineering

- [Ohm's Law Calculator](https://www.digikey.com/en/resources/conversion-calculators/conversion-calculator-ohms) — DigiKey Ohm's law tool.
- [Ideal Gas Law Calculator](https://www.omnicalculator.com/physics/ideal-gas-law) — PV=nRT calculations.
- [Velocity Calculator](https://calnex.pro/en_US/calculators/physics/velocity_calculator) — Speed, distance, time with formulas & steps.
- [Gravity Calculator](https://calnex.pro/en_US/calculators/physics/gravity_calculator) — Gravitational force between two objects.
- [Density Calculator](https://www.calculatorsoup.com/calculators/physics/density.php) — Density, mass & volume.
- [WattsCalc](https://wattscalc.com) — Free wattage and electricity cost calculator for appliances and devices.

## Unit Conversion

- [Convert Units](https://www.convertunits.com/) — Comprehensive unit converter.
- [Metric Conversion](https://www.metric-conversions.org/) — Metric converter reference.
- [Calnex Converter Suite](https://calnex.pro/en_US/convert) — 50+ unit converters including length, weight, volume, temperature, area, speed, pressure, fuel economy.
- [Temperature Converter](https://www.rapidtables.com/convert/temperature/) — °C, °F, K conversion.

## Everyday Life

- [Tip Calculator](https://www.omnicalculator.com/finance/tip) — Restaurant tip percentage.
- [Date Calculator](https://www.timeanddate.com/date/dateadd.html) — Date arithmetic.
- [Sleep Cycle Calculator](https://calnex.pro/en_US/calculators/daily/sleep_cycle_calculator) — Optimal wake-up time based on sleep cycles.
- [Water Intake Calculator](https://calnex.pro/en_US/calculators/daily/water_intake_calculator) — Daily hydration needs based on weight & activity.
- [Random Number Generator](https://www.random.org/) — True random number generation.
- [QR Code Generator](https://calnex.pro/en_US/tools/qr_code_generator) — Custom QR code with colors & sizes.
- [Lawn Mowing Cost Calculator](https://lawnmowingcalculator.com) — Free calculator for lawn mowing cost estimates by area, rate, and extras.
- [Embroidery Pricing Calculator](https://embroiderypricingcalculator.com) — Free embroidery job pricing from stitches, fabric, and labor.

## Privacy-First Tools

_Tools that process everything locally — no data ever leaves your device._

- [**Calnex**](https://calnex.pro) — 147+ calculators running 100% locally in the browser. Zero data upload, no tracking, no registration.
- [Qalculate!](https://qalculate.github.io/) — Desktop multi-purpose calculator (open source).
- [Numi](https://numi.app/) — Beautiful desktop calculator app with offline privacy.

---

## Contributing

Contributions welcome! Please read [CONTRIBUTING.md](CONTRIBUTING.md) first.

**Guidelines:**
- Keep entries in alphabetical order within categories
- Include a short, accurate description
- No affiliate links or paid placements
- Tools must be free or have a meaningful free tier

---

## License

[![CC0](https://mirrors.creativecommons.org/presskit/buttons/88x31/svg/cc-zero.svg)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, the author has waived all copyright and related or neighboring rights to this work.